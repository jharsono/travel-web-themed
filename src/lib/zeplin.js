...
const STYLEGUIDE_ID = import.meta.env.VITE_STYLEGUIDE_ID;
...
export const getStyleguideColors = async () => {
	const _getColors = async (resolve, reject) => {
		try {
			const { data } = await zeplin.colors.getStyleguideColors(STYLEGUIDE_ID);
			resolve(data);
		} catch (error) {
			reject(error)
		}
	}
	const styleguideColors = await new Promise((resolve, reject) => _getColors(resolve, reject))
	
	// Format the styleguide color model from API to the colors object expected by Tailwind
    const colors = {}
    styleguideColors.map(color => {
        const { name, r, g, b} = color;
        colors[name.toLowerCase()] = `rgb(${r},${g},${b})`;
